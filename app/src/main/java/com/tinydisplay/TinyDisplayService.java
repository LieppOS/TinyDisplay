...
public class TinyDisplayService extends Service {
    ...
    public static final String ACTION_KEY_F1 = "com.tinydisplay.action.KEY_F1";
    public static final String ACTION_KEY_F2 = "com.tinydisplay.action.KEY_F2";
    ...
    /** Handle an F1/F2 hardware key; capture a real photo if it is the chosen shutter. */
    private void onShutterKey(String which) {
        String key = prefs.getString("photo_key", "f2");
        if (!which.equals(key)) { Log.i(TAG, "Key " + which + " ignored (shutter=" + key + ")"); return; }
        renderHandler.post(this::takePhoto);
    }

    private void takePhoto() {
        if (photoInProgress) return;
        photoInProgress = true;
        Log.i(TAG, "Photo requested");
        pocketCovered = false;
        if (!subScreenPowered) powerOnSubScreen();
        stopCameraModeLocked();
        pushFrame(RawFontRenderer.renderText("PHOTO", 5));
        int facing = CameraCharacteristics.LENS_FACING_FRONT;
        PhotoCapturer.capture(this, facing, new PhotoCapturer.Result() {
            @Override public void onSaved(Uri uri) {
                Log.i(TAG, "Photo saved: " + uri);
                renderHandler.post(() -> {
                    photoInProgress = false;
                    pushFrame(RawFontRenderer.renderText("SAVED", 5));
                    currentPage = PAGE_CLOCK;
                    scheduleClockUpdate(1500);
                });
            }
            @Override public void onError(String message, Throwable t) {
                Log.w(TAG, "Photo failed: " + message, t);
                renderHandler.post(() -> {
                    photoInProgress = false;
                    pushFrame(RawFontRenderer.renderText("PHOTO ERR", 3));
                    currentPage = PAGE_CLOCK;
                    scheduleClockUpdate(1500);
                });
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ACTION_KEY_F1.equals(action)) { onShutterKey("f1"); return START_STICKY; }
        if (ACTION_KEY_F2.equals(action) || ACTION_F2_PHOTO.equals(action)) { onShutterKey("f2"); return START_STICKY; }
